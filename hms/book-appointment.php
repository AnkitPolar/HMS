...
if(isset($_POST['ajax']) && $_POST['ajax'] == 1) {
	...
	if($query) {
		...
		echo json_encode(['success' => true, 'appointment_id' => $appointment_id, 'fees' => $fees]);
	} else {
...
$docstatus=1;
	...
	if($query)
	{
		$appointment_id = mysql_insert_id(); // Get the last inserted appointment ID
		echo "<script>window.location.href='pay.php?appointment_id=$appointment_id';</script>";
		exit();
	}
...
<script>
document.querySelector('form[name="book"]').addEventListener('submit', function(e) {
    e.preventDefault();
    var form = this;
    ...
    .then(data => {
        if(data.success) {
            ...
            var options = {
                "key": "rzp_test_KfTKbgGeTHoTEN", // Your NEW Razorpay Key ID
                "amount": data.fees * 100, // Amount in paise
                "currency": "INR",
                ...
                "handler": function (response){
                    ...
                    payForm.method = 'POST';
                    payForm.action = 'payment_success.php';
                    payForm.innerHTML = '<input name="razorpay_payment_id" value="'+response.razorpay_payment_id+'"/>' +
                                        '<input name="appointment_id" value="'+data.appointment_id+'"/>';
